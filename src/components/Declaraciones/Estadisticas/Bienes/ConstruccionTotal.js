...
class ConstruccionTotal extends Component {
  ...
  componentDidMount() {
    fetch(
      ConstClass.estadisticas + "/api/v1/estadisticas/superficie-construccion"
    )
      ...
      .then(d => {
        ...
        this.setState({
          methods: {
            ...
            shapeConfig: {
              label: false,
              fill: (d, i) => ConstClass.colorsChart[(i + 1) % 12]
            },
            legend: true,
            axes: {
              fill: "#666672"
        ...
        <Grid item xs={12}>
          <Paper className={classes.paper}>
            <Typography className={classes.titulo}>
              Funcionarios por propiedad (área de construcción total)
            </Typography>
...
export default withStyles(styles)(ConstruccionTotal);
