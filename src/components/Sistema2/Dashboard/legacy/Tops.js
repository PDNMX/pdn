...
let color = ['#F87268', '#DC6AF0', '#B286FD', '#8A97D6', '#3DA2F5', '#00BCD4', '#00B3A1', '#4CAF50', '#8BC34A', '#FFC107', '#FF9800', '#FF7247', '#E2977E', '#FD938B', '#FF85AD', '#ED85FF', '#AD94D6', '#8A9BF9', '#6DBCFD', '#1C9BFD', '#1DE2FC', '#00DBC5', '#71E575', '#AFEE68', '#F9AE3E', '#FF9270', '#F2B39C'];
          ...
          <Grid container>
            ...
            <Grid item md={3} xs={12}>
              <FormControl className={classes.formControl}>
                ...
                <Select style={{ marginTop: '0px' }} value={ejercicio} onChange={e => setEjercicio(e.target.value)} label={'Ejercicio'}>
                  ...
                  {ejercicios.map(item => {
                    return (
                      <MenuItem key={item.ejercicio} value={item.ejercicio}>
                        {item.ejercicio}
                      </MenuItem>
                    );
                  })}
            ...
            <Grid item md={3} xs={12}>
              <FormControl className={classes.formControl}>
                ...
                <Select style={{ marginTop: '0px' }} value={ramo} onChange={e => setRamo(e.target.value)} label={'Ramo'}>
                  <MenuItem value={'any'}>TODOS</MenuItem>
                  {ramos.map(item => {
                    return (
                      <MenuItem value={item.ramo} key={item.ramo}>
                        {item.ramo}
                      </MenuItem>
                    );
                  })}
            ...
            <Grid item md={3} xs={12}>
              <FormControl className={classes.formControl}>
                ...
                <Select style={{ marginTop: '0px' }} value={institucion} onChange={e => setInstitucion(e.target.value)} label={'Institución'}>
                  <MenuItem value={'any'}>TODAS</MenuItem>
                  {instituciones.map(item => {
                    return (
                      <MenuItem value={item.institucion} key={item.institucion}>
                        {item.institucion}
                      </MenuItem>
                    );
                  })}
            ...
            <Grid item md={3} xs={12}>
              <FormControl className={classes.formControl}>
                ...
                <Select style={{ marginTop: '0px' }} value={top} onChange={e => setTop(e.target.value)} label={'Top'}>
                  <MenuItem key={0} value={'id_procedimiento'}>
                    PROCEDIMIENTO
                  </MenuItem>
                  <MenuItem key={1} value={'INSTITUCION'}>
                    INSTITUCIONES
                  </MenuItem>
                  <MenuItem key={2} value={'UR'}>
                    UNIDADES RESPONSABLES
                  </MenuItem>
                  <MenuItem key={3} value={'PUESTO'}>
                    PUESTOS
                  </MenuItem>
                </Select>
              </FormControl>
            ...
            <Grid item xs={12} id={'grafica'}>
              {methods && methods.data && <BarChart config={methods} />}
              {error && <Alert severity='error'>No disponible por el momento, intente más tarde.</Alert>}
            </Grid>
          </Grid>
        </Grid>
      </ContainerChart>
